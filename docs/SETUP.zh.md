...
2. 回到超级群：应出现一个**新话题**，里面有用户消息；若命中话术/知识库，还会有 💡 Jev 建议（**最多三条可选**）和「发送① / 发送② / 发送③」按钮
3. 在话题里回复几句 → 用户私聊应收到
4. 按需编辑：
...
客服与客户真实问答可以变成店铺话术/知识库：

1. 客户私聊发**文字**，客服在话题里回复（或点 Jev「发送①/②/③」）且**成功送达**
2. 管理员私聊 Bot 发 **`/learn`**，看到待审核问答
3. 点 **入库话术**（写入 `data/scripts.json`）或 **入库知识**（写入 `data/knowledge.md`），或 **忽略**
4. 入库后会热重载 Jev 匹配（一般不必为学习条目单独重启）

说明：本版本只保存**原始问答**，**不调用模型总结**；后续版本可能会加。可选在 `.env` 设置 `LEARN_ADMIN_IDS=`（逗号分隔的 user id）限制谁能审核；留空则任意私聊用户可 `/learn`。
